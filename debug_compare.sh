# -*- coding: utf-8 -*-
import json, math, heapq, subprocess, os, textwrap
...
st_by_id={s['id']:s for s in stops}
...
def road_report(target, limit=10):
    print('\nROAD',target)
    for rid,name,hw,pts in roads:
        if target in name:
            print('road',rid,name,hw)
            for lat,lon in pts[:min(limit,len(pts))]:
                i=node_id.get(key(lat,lon))
                print(f'{lat:.7f} {lon:.7f} | nearest {fmt(best_nearest.get(i))} | total {fmt(best_total.get(i))}')
            break
road_report('Брюсова')
road_report('Казахская')
road_report('Ибрагимова')
print('\nPLOSCHAD VOSSTANIYA / VOLGOGRADSKAYA / IBRAGIMOVA STOPS')
for s in stops:
    if any(x in s['name'] for x in ['пл. Восстания','Волгоградская','Ибрагимова','Восстания (Ибрагимова)']):
        print(s['id'],s['name'],'transport',round(s['transport']/60,1),'latlon',s['lat'],s['lon'])
print('\nTARGET STOP TRANSPORTS')
for sid in ['11768','11792','12042','12082','12107','12382','11932','12044','12036','12035','12386']:
    s=st_by_id.get(sid)
    if s: print(sid,s['name'],round(s['transport']/60,1),s['lat'],s['lon'])
PY
python3 /tmp/debug_compare.py
