...
def dijkstra(seed_stops, mode):
    ...
    return best
print('building dijkstra')
best_total=dijkstra(stops,'total')
best_nearest=dijkstra(stops,'walk')
best_origin_walk=dijkstra([s for s in stops if s['id'] in origins],'walk')
def fmt(r):
    if not r: return 'none'
    st=r['stop']
    return f"{st['id']} {st['name']} transport={st['transport']/60:.1f} walk={r['walk']/WALK_SPEED/60:.1f} total={r['total']/60:.1f} walkm={r['walk']:.0f}"
def rec(lat,lon,best):
    i=node_id.get(key(lat,lon)); return best.get(i) if i is not None else None
print('\nVOSHOD nearest nodes')
voshod=[s for s in stops if 'Восход' in s['name']]
seen=[]
for s in voshod:
    for i,(lat,lon) in enumerate(nodes):
        dist=hav(s['lat'],s['lon'],lat,lon)
        if dist<=70: seen.append((dist,i,lat,lon,s['id'],s['name']))
for dist,i,lat,lon,sid,sname in sorted(seen)[:12]:
    rt=best_total.get(i); ow=best_origin_walk.get(i)
    print(f'{lat:.7f} {lon:.7f} near {sid} {sname} dist={dist:.0f}m | totalBest {fmt(rt)} | originWalkOnly {fmt(ow)}')
print('\nROADS BRYUSOVA / KAZAKHSKAYA')
for target in ['Брюсова','Казахская']:
    print('\n--',target)
    n=0
    for rid,name,hw,pts in roads:
        if target in name:
            print('road',rid,name,hw,'points',len(pts))
            for lat,lon in pts[:min(12,len(pts))]:
                i=node_id.get(key(lat,lon));
                print(f'{lat:.7f} {lon:.7f} | nearestStop {fmt(best_nearest.get(i))} | totalBest {fmt(best_total.get(i))}')
            n+=1
            if n>=6: break
print('\nSTOPS around Bryusova/Kazakhskaya names')
for s in stops:
    if any(x in s['name'] for x in ['Брюсова','Казах','Восход','Социально','Тунакова','Гагарина','Декабристов','Ибрагимова']):
        print(s['id'],s['name'],round(s['transport']/60,1),s['lat'],s['lon'])
