...
The job stores progress per `serviceDate|departureBucketMinute` and writes `journeys`, `journey-fragments`, and `request-grid-counts` partitioned by `serviceDate/departureBucketMinute`. This is deliberate: a full service day is too large to treat as one all-or-nothing write. The nightly service should run continuously inside the quiet window and stop before daytime ingestion resumes. Defaults are `BUS_TRANSFER_MAX_BUCKETS_PER_RUN=100000`, `BUS_TRANSFER_STOP_BEFORE_LOCAL_TIME=03:55`, and `BUS_TRANSFER_MAX_CANDIDATE_EVENTS_PER_STOP=1`; this makes the job process as much backlog as possible between 00:00 and 04:00 while still creating durable bucket-level progress. Raise the candidate count only after measuring runtime and memory on production data.
