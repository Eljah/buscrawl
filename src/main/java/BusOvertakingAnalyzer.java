...
public class BusOvertakingAnalyzer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        spark.udf().register("haversine", (UDF4<Double, Double, Double, Double, Double>)
                BusOvertakingAnalyzer::haversine, DataTypes.DoubleType);
        ...
        Dataset<Row> overtakes;
        if (Files.exists(Paths.get("D:/parquet/overtakes.parquet"))) {
            overtakes = spark.read().parquet("D:/parquet/overtakes.parquet");
        } else {
            overtakes = spark.sql(
                    "SELECT later.plate AS overtaker, earlier.plate AS overtaken, " +
                            "later.start_stop, later.end_stop, later.routeId, later.direction, " +
                            "later.departure_time AS later_departure, earlier.departure_time AS earlier_departure, " +
                            "later.arrival_time AS later_arrival, earlier.arrival_time AS earlier_arrival " +
                            "FROM segments earlier JOIN segments later ON earlier.start_stop = later.start_stop " +
                            "AND earlier.end_stop = later.end_stop AND earlier.routeId = later.routeId " +
                            "AND earlier.direction = later.direction " +
                            "WHERE later.departure_time > earlier.departure_time AND later.arrival_time < earlier.arrival_time"
            ).checkpoint();
            overtakes.write().mode("overwrite").parquet("D:/parquet/overtakes.parquet");
        }
        overtakes.createOrReplaceTempView("overtakes");
        overtakes.show(5, false);

        System.out.println("=== ТОП-10 автобусов по количеству обгонов ===");
        spark.sql("SELECT overtaker, COUNT(*) AS cnt FROM overtakes GROUP BY overtaker ORDER BY cnt DESC LIMIT 10")
                .show(false);

        System.out.println("=== ТОП-10 маршрутов по количеству обгонов ===");
        spark.sql("SELECT routeId, direction, COUNT(*) AS cnt FROM overtakes GROUP BY routeId, direction ORDER BY cnt DESC LIMIT 10")
                .show(false);
