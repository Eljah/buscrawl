...
public class BusSegmentsHeatmapNoLegend30Min {
    public static void main(String[] args) {
        SparkSession spark = SparkSession.builder()
                .appName("BusSegmentsHeatmapNoLegend30Min")
                .master("local[*]")
                .getOrCreate();

        Dataset<Row> speedStats = spark.read().parquet("D:/parquet/speedStats.parquet");

        // Оставляем только сегменты, где время между точками менее 30 минут
        Dataset<Row> filtered = speedStats.filter("fast_duration_sec < 1800 AND slow_duration_sec < 1800");

        // Сортируем датасет по speed_ratio по возрастанию
        List<Row> segments = filtered.select(
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Тепловая карта скоростей сегментов автобусов без легенды",
                "Долгота",
                "Широта",
                dataset
        );
